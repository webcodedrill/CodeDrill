...
async function apiSyncStats({ type, count = 1 }) {
  try {
    const body = {};
    if (type === 'mcq')       body.mcqsSolved       = count;
    if (type === 'question')  body.questionsDone     = count;
    if (type === 'paragraph') body.paragraphsFixed   = count;
    await apiPost('/api/user/sync-stats', body);
  } catch(e) {}
}
