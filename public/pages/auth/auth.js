...
async function doLogin() {
  ...
  try {
    ...
    const matched = verifyLogin(email, pass);
    if (!matched) return setAuthState({ error: 'Incorrect email or password!' });
    setUser({ name: matched.name, username: matched.username, email: matched.email, userId: matched.userId });
    window.location.href = '../home/index.html';
  }
}
...
function doRegister() {
  ...
  /* ── Duplicate checks (backend will also check, but fast local check) ── */
  if (findUserByEmail(email))       return setAuthState({ error: 'An account with this email already exists!' });
  if (findUserByUsername(username)) return setAuthState({ error: 'This username is already taken!' });
  if (phone && findUserByPhone(phone)) return setAuthState({ error: 'An account with this phone number already exists!' });
  if (findUserByName(name))         return setAuthState({ error: 'An account with this name already exists!' });
  ...
  try {
    ...
    const record = registerUser({ name, username, email, phone, password: pass });
    setUser({ name: record.name, username: record.username, email: record.email, userId: d.user?.userId || record.userId });
    if (inviteCode) {
      const referrer = findUserByUsername(inviteCode);
      if (referrer) {
        addToTeam(referrer.userId, { userId: record.userId, username: record.username, name: record.name });
        saveInvitedBy(record.userId, referrer.userId);
      }
    }
    window.location.href = '../home/index.html';
  } catch(e) {
    ...
    const record = registerUser({ name, username, email, phone, password: pass });
    setUser({ name: record.name, username: record.username, email: record.email, userId: record.userId });
    if (inviteCode) {
      const referrer = findUserByUsername(inviteCode);
      if (referrer) {
        addToTeam(referrer.userId, { userId: record.userId, username: record.username, name: record.name });
...
function doForgot() {
  ...
  if (!existing) return setAuthState({ error: 'No account found with this email!' });
...
async function doResetPassword() {
  ...
  try {
    await apiForgotPassword(authState.forgotEmail, newPass);
  } catch(e) {}
  updatePasswordByEmail(authState.forgotEmail, newPass); // localStorage fallback
