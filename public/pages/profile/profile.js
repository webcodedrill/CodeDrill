...
(function init() {
  ...
  /* Load real stats + rank from backend */
  if (typeof apiGet === 'function') {
    ...
    // Rank bhi load karo
    apiGet('/api/user/all-users').then(data => {
      const rankEl = document.getElementById('stat-rank');
      if (!rankEl) return;
      const users = (data && data.users) ? data.users : [];
      if (users.length === 0) { rankEl.textContent = '—'; return; }

      // Har user ka best final test score nikalo
      function getBest(u) {
        const attempts = u.attempts || u.finalTestAttempts || [];
        if (!attempts.length) return -1;
        return Math.max(...attempts.map(a => (a.grading && a.grading.totalMarks) || a.total || 0));
      }

      const sorted = [...users].sort((a, b) => getBest(b) - getBest(a));
      const myUser = user;
      const myIdx  = sorted.findIndex(u => u._id === myUser.userId || u.userId === myUser.userId || u.email === myUser.email);
