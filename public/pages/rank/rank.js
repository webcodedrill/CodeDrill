...
/* ── Safe attempt reader — from backend user object ── */
function safeGetAttempts(regUser) {
  try {
    // Backend user object mein attempts array hota hai
    if (regUser && Array.isArray(regUser.attempts)) return regUser.attempts;
    if (regUser && Array.isArray(regUser.finalTestAttempts)) return regUser.finalTestAttempts;
    return [];
  } catch (e) {
    return [];
  }
...
function buildLeaderboard(currentUserId) {
  let allUsers = [];
  try { allUsers = window._rankAllUsers || []; } catch(e) {}
...
function renderRankPage(user) {
  try {
    ...
    const total  = (window._rankAllUsers || []).length;
...
(async function init() {
  try {
    ...
    // Loading dikhao
    document.getElementById('page-content').innerHTML =
      '<div style="padding:60px;text-align:center;color:#8B92A5">⏳ Leaderboard load ho raha hai...</div>';

    // Backend se sab users lo
    try {
      const data = await apiGetAllUsers();
      window._rankAllUsers = (data && data.users) ? data.users : [];
    } catch(e) {
      window._rankAllUsers = [];
    }
